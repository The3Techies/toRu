...
export default function Benefits() {
    ...
    const HtmlCol2: Array<JSX.Element> = col2.map((num, i) => {
        return (
            <div key={i} className="col-start-6 col-end-6 flex flex-col items-center select-none">
    ...
    const HtmlCol3: Array<JSX.Element> = col3.map(({Title, Par, Row }, i) => {
        return (
            <div className={"col-start-7 col-end-10 " + Row }>
            <h1 className={'font-Cairo text-xl text-center ' + state.fontColor}>{Title}</h1>
            <p className={'font-Cairo text-sm leading-relaxed  text-center font-light ' + state.fontColor}>{Par}</p>
        </div>
        )
    ...
    return (
        <div className="w-full mt-14 md:mt-0 flex justify-center ">
            <div className={" shadow-lg rounded-2xl w-full max-w-6xl grid grid-cols-11  grid-rows-12 py-10 " + state.bgColor + state.shadowColor}>
               {HtmlCol1}
               {HtmlCol2}
               {HtmlCol3}
            </div>
